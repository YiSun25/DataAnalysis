# -*- coding: utf-8 -*-
"""
Created on Sat Jun 29 22:28:25 2024
...
def TF22_gen(n):
    ...
    # print(TF_array_accu)
    # TF_array_accu =  [list(reversed(column)) for column in zip(*TF_array_accu)]
    TF_array_accu = np.flipud(TF_array_accu)#.T).T # 每行存储不同stage的TW，这里将行进行翻转，TF就从DIF的形式变成DIT的形式
    # print(TF_array_accu)
    # TF_array_fixed =  Fxp(TF_array_accu, True, n_word_TF, n_frac_TF)
    # print(TF_array_fixed)
    return TF_array_accu#, TF_array_fixed


       
# DIF radix-2 BF
def BF_r22(in0, in1, TF0, TF1):
    # print(in0, in1, TF)
    out0 = complex(TF0*in0 + TF1*in1)/2 # multiply 1/2 for scaling, to avoid overflow, when do scaling in fixed point, note the 
    out1 = complex(TF0*in0 - TF1*in1)/2 
    # print(out0,out1)
    return out0, out1
    ...
    for i in range(stages):
        ...
        for j in range(N//n):
            for k in range (step_len):
                # print('i',i, OUT_vec_accu[n*j+k][i], OUT_vec_accu[n*j+k+step_len][i],TF_array[i][n*j+k], TF_array[i][n*j+k+step_len])
                OUT_vec_accu[n*j+k][i+1], OUT_vec_accu[n*j+k+step_len][i+1] =  BF_r22(OUT_vec_accu[n*j+k][i], OUT_vec_accu[n*j+k+step_len][i],TF_array[i][n*j+k], TF_array[i][n*j+k+step_len])
                # print(OUT_vec_accu[n*j+k][i+1], OUT_vec_accu[n*j+k+step_len][i+1])
    final_res = OUT_vec_accu[:,-1]
    
    return OUT_vec_accu, final_res

    
# Fixed point FFT implementation    
def FFT_Fixed(IN_vec_fixed, TF_array, approx=0, appt_stage=0, Frac_Appr=0):
    OUT_vec_fxp = np.zeros((N,stages+1), dtype = np.cdouble)
    OUT_vec_fxp[:,0] = IN_vec_fixed
    ...
    for i in range(stages):
        ...
        for j in range(N//n):
            for k in range (step_len):
                TF_array[i][n*j+k] = Fxp(TF_array[i][n*j+k], True, n_word_TF, n_frac_TF)
                TF_array[i][n*j+k+step_len] = Fxp(TF_array[i][n*j+k+step_len], True, n_word_TF, n_frac_TF)
                
                rr0, ri0, ir0, ii0 = multiplication(OUT_vec_fxp[n*j+k][i], TF_array[i][n*j+k])
                # add tiny bias 1e-16 to activate round half up, then the method will not be round to even.
                rr0 = Fxp(rr0+tiny_bias, True, n_word, n_word - 1, rounding = method)
                ri0 = Fxp(ri0+tiny_bias, True, n_word, n_word - 1, rounding = method)
                ir0 = Fxp(ir0+tiny_bias, True, n_word, n_word - 1, rounding = method)
                ii0 = Fxp(ii0+tiny_bias, True, n_word, n_word - 1, rounding = method)
                
                rr1, ri1, ir1, ii1 = multiplication(OUT_vec_fxp[n*j+k+step_len][i],TF_array[i][n*j+k+step_len])
                rr1 = Fxp(rr1+tiny_bias, True, n_word, n_word - 1, rounding = method)
                ri1 = Fxp(ri1+tiny_bias, True, n_word, n_word - 1, rounding = method)
                ir1 = Fxp(ir1+tiny_bias, True, n_word, n_word - 1, rounding = method)
                ii1 = Fxp(ii1+tiny_bias, True, n_word, n_word - 1, rounding = method)
                
                # print('i',i, OUT_vec_fxp[n*j+k][i], OUT_vec_fxp[n*j+k+step_len][i],TF_array_fixed[i][n*j+k], TF_array_fixed[i][n*j+k+step_len])
                OUT_vec_fxp[n*j+k][i+1] = (rr0-ii0)+1j*(ri0+ir0) + (rr1-ii1)+1j*(ri1+ir1)
                OUT_vec_fxp[n*j+k+step_len][i+1] = (rr0-ii0)+1j*(ri0+ir0) - ((rr1-ii1)+1j*(ri1+ir1))
                
                # change back to fixed point
                ...
                if (counter == appt_stage):
                    if (appr == 1):  # approximation stage number
                        OUT_vec_fxp[n*j+k][i+1] = OUT_vec_fxp[n*j+k][i+1]+(1+1j)*tiny_bias
...
def check():
        ...
        flp_matri = FFT_Accuracy(Xn_R, TF_array)[0]
        fixed_matri = FFT_Fixed(Xn_R_fixed, TF_array)[0]
...
def main():
    for seed in range (1,100):
        ...
        Xn_R_fixed = Fxp(Xn_R+1e-16, True, n_word, n_word-1, rounding = method)
        ...
        for j in range (8,15): # if max n_frac_appr equals to 7, then j is from 1 to 7
            for i in range (stages):
                appr_list[j-8][:,i] = FFT_Fixed(Xn_R_fixed, TF_array, 1, i, j)[1]
