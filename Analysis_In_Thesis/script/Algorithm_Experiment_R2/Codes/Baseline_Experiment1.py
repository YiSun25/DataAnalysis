...
for WordLen in range(16, 0, -1):
    ...
    for In_vec in random_inputs:
        ...
        SQNR_list.append(FFT_R2.SNR_flp_fxp) # 将每一个不同的输入计算得来的SQNR放入list中
...
print(result1)
# save results1
res_folder = f"Results_Folder_AftMid/Baseline1_Results_AftMid_RHU/N{FFT_R2.N}"
...
for stage in range(FFT_R2.stages-2): 
    if stage == 3:
        for D_Wordlen in range(16, 0, -1):
            # print(stage+1)
            TF_n_word[stage+1] = D_Wordlen # change TF wordlength and Output wordlength together
            ...
            # print(Data_n_word)
            for In_vec in random_inputs:
                FFT_R2.FFT_Accuracy(In_vec) # Not to be omitted here. 
                FFT_R2.FFT_Fixed(In_vec, In_n_word, Data_n_word, TF_n_word)
                FFT_R2.sqnr_calculation()
                # print(FFT_R2.SNR_flp_fxp)
