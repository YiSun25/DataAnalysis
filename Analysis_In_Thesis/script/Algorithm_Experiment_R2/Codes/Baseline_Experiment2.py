# -*- coding: utf-8 -*-
"""
Created on Tue Apr 30 16:50:10 2024

@author: yisss
"""

# In[1] Change All Data Wordlength and TF Wordlength together firstly
...
# change input data wordlength, intermediate data wordlength and TF wordlength together
for WordLen in range(16, 0, -1):   # reduce the wordlength of input data
    Data_n_word[:5] = [WordLen]*5  # Data_n_word 前五个位宽一起减
    TF_n_word = [WordLen]*6        # TF_n_word 六个位宽一起减
    
    SQNR_list=[]
    for In_vec in random_inputs:
        FFT_R2.TF_Gen()
        FFT_R2.FFT_Accuracy(In_vec)
        FFT_R2.FFT_Fixed(In_vec, WordLen, Data_n_word, TF_n_word)
    ...
    Data_n_word_list.append(Data_n_word.copy())  # 复制当前的Data_n_word, 存入Data_n_word_list中
    TF_n_word_list.append(TF_n_word.copy())      # 复制当前的TF_n_word, 存入TF_n_word_list中
    mean_SQNR_flp_fxp_list.append(mean_SQNR_flp_fxp)
    # print('Mean_Value', mean_SQNR_flp_fxp)
    # print(In_Wordlen)
    
    if mean_SQNR_flp_fxp < SQNR_Req:
        In_n_word = WordLen + 1                    # add one bit back to maintain the 45 dB
        Data_n_word[:5] = [WordLen+1]*5
        TF_n_word = [WordLen+1]*6
        break
    else:
        continue
...
result1 = [[a, b, c, d] for a, b, c, d in zip(In_Wordlen_list, Data_n_word_list, TF_n_word_list, mean_SQNR_flp_fxp_list)]
print(result1)
# print(In_n_word)
# print(Data_n_word)

# save result1
res_folder = f"Results_Folder/Baseline2_Results/N{FFT_R2.N}"
...
# In[2] Change Input Wordlength 0.stage TF wordlength 
...
for In_Wordlen in range(start_word, 0, -1):
    ...
    mean_SQNR_flp_fxp2 =  sum(SQNR_list2) / len(SQNR_list2)  # calculate average SNR
    ...
    if mean_SQNR_flp_fxp2 < SQNR_Req:
        ...
        TF_n_word[0] = In_Wordlen + 1
        break
    else: 
        continue

result2 = [[a, b, c] for a, b, c in zip(In_Wordlen_list, TF_n_word_list, mean_SQNR_flp_fxp_list2)]
print(result2)
...
# In[3] Change intermediate data wordlength and TF wordlength from stage1 to stage 5
# change data wordlength
print('*********',In_n_word)
print('*********', TF_n_word)
# FFT_R2.FFT_Accuracy(In_vec)
...
for stage in range(FFT_R2.stages-1): # skip last stage, last stage don't need to 
    for D_Wordlen in range(start_word, 0, -1): # reduce the data wordlengh from "start_word"
        Data_n_word[stage] = D_Wordlen
        TF_n_word[stage+1] = D_Wordlen
        SQNR_list3 = []
        # print(Data_n_word)
        for In_vec in random_inputs:
            FFT_R2.TF_Gen()
            FFT_R2.FFT_Accuracy(In_vec) # Not to be comitted here. 
            FFT_R2.FFT_Fixed(In_vec, In_n_word, Data_n_word, TF_n_word)
            FFT_R2.sqnr_calculation()
            # print(FFT_R2.SNR_flp_fxp)
...
result3 = [[a, b, c] for a, b, c in zip(Data_n_word_list, TF_n_word_list, mean_SQNR_flp_fxp_list3)]
print(result3)
...
# # In[4] Change TF Wordlength at last
# print('#######', In_n_word)
# print('#######', Data_n_word)
...
# # save results3
# # res_folder = f"Results_Folder/Data_Results/N{FFT_R2.N}"
# # if not os.path.exists(res_folder):
# #     os.makedirs(res_folder)
# # df_result3 = pd.DataFrame(result3)
# # filename_res3 = f"res3_{SQNR_Req}dB.csv"
